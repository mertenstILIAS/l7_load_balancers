...
resource "oci_load_balancer_backend_set" "these" {
  for_each = local.one_dimension_processed_l7_lb_backend_sets != null
    ? local.one_dimension_processed_l7_lb_backend_sets
    : {}
  ...
  # 🔁 FIXED ssl_configuration — only rendered if provided
  dynamic "ssl_configuration" {
    ...
    content {
      ...
      certificate_keys               = ssl_configuration.value.certificate_keys
      ...
      trusted_certificate_authority_keys = ssl_configuration.value.trusted_certificate_authority_keys
